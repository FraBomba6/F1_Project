...
Per quantificare il progresso tecnico è possibile effettuare un analisi dei tempi sul giro e verificare l'andamento di questi ultimi nel tempo.
...
## Confronto diretto
```{r}
com_circuit <- semi_join(
  filter(select(races, year, circuitId), year == 2004),
  filter(select(races, year, circuitId), year == 2019), by = "circuitId") %>%
  select(circuitId)

vs_lap <- lap_times %>%
  left_join(select(races, raceId, circuitId, year), by = "raceId") %>%
  left_join(select(circuits, circuitId, circuitRef), by = "circuitId") %>%
  semi_join(com_circuit, by = "circuitId") %>%
  ...
  sample_n(3) %>%
  unnest() %>%
  ggplot(aes(x = lap, y = milliseconds, group = year, color = circuitRef)) +
  geom_line()
ggplotly(vs_lap)
  
pit_stops %>%
  filter(driverId == 844 & raceId == 1023)
```
