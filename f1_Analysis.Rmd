...
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, out.height = "100%", out.width = "100%")
```
...
I singoli costruttori hanno dei ricavi annui che vanno dai 70 milioni USD ai __500 milioni USD__ a seconda di principalmente tre fattori:
...
La pandemia globale che stiamo affrontando ha messo in difficoltà anche il mondo dello sport e nemmeno la F1 non ha avuto scampo. Questo ha causato uno slittamento dell'inizio della stagione e una riduzione drastica dei circuiti a disposizione, costringendo la FIA ad organizzare gare multiple su alcuni circuiti (Spielberg, Silverstone e Manama), ad escludere circuiti di importanza storica come Monaco e ad assegnare all'Italia ben 3 gare su altrettanti circuiti (Monza, Imola e Mugello).

Ad oggi ci sono in calendario 17 gare e difficilmente se ne aggiungeranno altre.

La F1 ha saputo adattarsi in maniera egregia alle esigenze igienico-sanitarie che l'attuale situazione impone
...
leaflet(world_borders , options = leafletOptions(minZoom = 2), width = "100%") %>%  # Alcune opzioni per la mappa: zoom minimo e larghezza nella pagina
  addTiles() %>%
...
## Arricchire i campioni
Il df costruito in precedenza, contenente alcuni dati sui campioni del mondo viene arricchito con i dati dei df del dataset originale
```{r}
champions <- champions %>%
...
Sono stati introdotti due ulteriori indici:
