...
I GP (Gran Premio) si sviluppano su tre giornate:

+il venerdì si svolgono due prove libere;
+il sabato viene eseguita un ulteriore prova libera e si tengongo le qualifiche;
+la domenica si corre la gara.
...
Il report finanziario annuale 2019 di Liberty Media (proprietaria della F1) riporta un ricavo di __2,022 miliardi USD__ (pagina 96 del [report](https://ir.libertymedia.com/static-files/2aeaccfa-a5e5-4fd4-abd1-eaa9dad5d6c0)).
...
## I team 2020
```{r 2020_teams, out.height = "100%", out.width = "100%"}
...
constructor_standings %>%
  semi_join(filter(races, year == 2020), by = "raceId") %>%
  left_join(select(races, year, round, raceId), by = "raceId") %>%
  left_join(select(constructors, constructorId, constructor = name), by = "constructorId") %>%
  select(-constructorStandingsId, -positionText) %>%
  arrange(desc(constructor)) %>%
  plot_ly(
    x = ~position,
    y = ~points,
    size = ~wins,
    color = ~constructor,
    colors = c("#9B0000", "#469BFF", "#DC0000", "#F0D787", "#FF8700", "#000000", "#F596C8", "#1E41FF", "#FFF500", "#00D2BE"),
    frame = ~round,
    text = ~constructor,
    hovertemplate = paste(
                        '<br>%{y}<br>'),
...
## I piloti 2020
```{r 2020_drivers, out.height = "100%", out.width = "100%"}
...
driver_standings %>%
  semi_join(filter(races, year == 2020), by = "raceId") %>%
  left_join(select(races, year, round, raceId), by = "raceId") %>%
  left_join(select(drivers, driverId, forename, surname), by = "driverId") %>%
  left_join(select(results, driverId, raceId, constructorId), by = c("raceId", "driverId")) %>%
  left_join(select(constructors, constructorId, constructor = name), by = "constructorId") %>%
...
Da allora la F1 ha totalizzato __`r races %>% filter(date < as.character(Sys.Date())) %>% select(raceId) %>% nrow()`__ gare (numero generato automaticamente a partire dal df __races__)
...
## Digressione: Italia, Paese record della F1
...
## I vincitori
Il primo passo è isolare da tutti i risultati delle gare (contenuti nella tabella __results__) i risultati relativi ai primi classificati (i vincitori della gara)
```{r winners}
winners <- results %>%
...
Partendo dai due data frame ottenuti in precedenza si può ora sapere chi ha vinto l'ultima occorrenza di una gara
```{r last_winners}
...
```
+ ai dati del __.geojson__ vanno uniti i dati relativi al numero di GP ospitati dagli stati per poterli integrare al meglio nella mappa
...
## Tecninca al limite
In 70 anni di storia, la F1 è diventata famosa per aver spinto al massimo i limiti tecnico-ingegneristici della costruzione delle automobili da corsa (ai progressi pionieristici della F1 in ambito di sistemi di propulsione si deve la nascita dei sistemi ibridi che troviamo sulle auto commerciali odierne).

Per provare a visualizzare il progresso tecnico è possibile effettuare un analisi dei tempi sul giro e verificare l'andamento di questi ultimi nel tempo.

## Circuito di Suzuka
Per effetturare il confronto dei tempi è stato deciso di utilizzare il Circuito di Suzuka
![](https://upload.wikimedia.org/wikipedia/commons/thumb/e/ec/Suzuka_circuit_map--2005.svg/1024px-Suzuka_circuit_map--2005.svg.png)
La scelta è dovuta al fatto che la conformazione del circuito non ha subito grandi variazioni nel tempo e quindi risulta efficace nel escludere la variabile delle modifiche al circuito nella valutazione dei tempi sul giro.
Dal punto di vista tecnico il circuito risulta essere bilanciato:

+ setup aerodinamico medio che garantisca aderenza nelle numerose curve veloci e una buona velocità di punta nei rettilinei;
+ la power unit opera al massimo regime per il 70% del giro;
+ i freni sono sottoposti ad uno sforzo medio basso per l'assenza di frenate brusche e il numero ridotto delle frenate stesse.

## Tempi in gara
```{r suzuka laps, out.height = "100%", out.width = "100%"}
suzuka_laps <- lap_times %>%
...
suzuka_laps %>%
  ...
  geom_boxplot() +
  stat_summary(fun = min, geom = "line", colour = "red", group = 1) +
  scale_y_continuous(breaks = seq(90000, 116000, 2000), limits = as.numeric(limits)) +
  ylab("Lap time [millisecoinds]") +
  xlab("Year") +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 90)) 
```
Dal grafico precedente risulta interessante osservare come nel 2015 il giro più veloce registrato in gara sia nettamente migliore di tutti i tempi sul giro registrati tra il 2006 e il 2018 (sempre durante una gara)

Questo è dovuto al fatto che tra il 2004 e il 2005 lo sviluppo del motore, in termini di erogazione di potenza, cavalli e rpm, ha raggiunto un apice nello sport: nel 2005 Honda ha prodotto un motore termico capce di erogare 920 CV (720 kW) su una monoposto che pesa 600 Kg.

Diversi incidenti hanno però portato la FIA a porsi domande sull'effettiva necessità di avere simili prestazioni a costo di sacrficare la sicurezza di piloti, meccanici, steward e pubblico. Questo ha portato ad un inasprimento delle regole sulle motorizzazioni, favorendo lo sviluppo della parte aerodinamica della vettura (nettamente carente rispetto ai propulsori) e degli pneumatici.

![F2004](./data/F2004.jpg){width=100%, fig.align='center'} ![SF90](./data/SF90.jpg){width=100%, fig.align='center'}

## Circuiti ad alto carico aerodinamico
A conferma di quanto detto in precedenza in merito al basso sviluppo aerodinamico delle vetture del 2004/2005, è interessante confrontare i tempi di qualifica su un circuito dove la potenza è marginale rispetto al controllo dell'auto.

Definito _Monaco senza i palazzi_ l' __Hungaroring__ è il circuito che dal 1986 ospita il GP di Ungheria. E' caratterizzato dalle complesse curve miste (veloci e lente) e dai brevi rettilinei, questo costringe i costruttori ad adottare setup aerodinamici che generino altissimi livelli di portanza per permettere alle auto di mantenere velocità adeguate nelle curve rinunciando alle velocità di punta sui rettilinei.

![](https://upload.wikimedia.org/wikipedia/commons/thumb/9/91/Hungaroring.svg/569px-Hungaroring.svg.png)


## Tempi in qualifica
Mentre in gara si attuano delle strategie di conservazione di pneumatici e motore, in qualifica si cerca di massimizzare le prestazioni sui singoli giri, il ché fa emergere anche le carenze tecniche delle vetture. Per questo motivo il seguente grafico si basa sui tempi dei migliori tempi in qualifica.
```{r suzuka q, out.height = "100%", out.width = "100%"}
hungary_q <- qualifying %>%
  left_join(select(races, raceId, circuitId, year), by = "raceId") %>%
  left_join(select(circuits, circuitId, circuitRef), by = "circuitId") %>%
  filter(circuitRef == "hungaroring") %>%
...
hungary_q$milliseconds = toSeconds(hungary_q$time)
...
hungary_q %>%
  ...
  geom_boxplot() +
  stat_summary(fun = min, geom = "line", colour = "red", group = 1) +
  scale_y_continuous(breaks = seq(74000, 98000, 2000), limits = as.numeric(limits)) +
  ylab("Lap time [millisecoinds]") +
  xlab("Year") +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 90))
```
Dal grafico precedente si può confermare quanto detto in precedenza: le vetture del 2005 pur essendo, a livello motoristico, più performanti di auto più recenti (per esempio la Red Bull 2010 con motore Renault da 750 CV), su circuiti dove l'aerodinamica ha maggior importanza non riescono ad esprimere la loro superiorità.

Un'altra informazione interessante che si può estrapolare dal precedente grafico è che nel 2016 e nel 2018 con molta probabilità le qualifiche si sono svolte con condizioni meteo miste. Infatti, la scatola ha un estensione notevole, indice del fatto che i tempi registrati sono molto vari.

## Confronto diretto di alcuni tracciati
```{r vs laps, out.height = "100%", out.width = "100%"}
vs_lap <- lap_times %>%
  left_join(select(races, raceId, circuitId, year), by = "raceId") %>%
  left_join(select(circuits, circuitId, circuitRef), by = "circuitId") %>%
  left_join(select(drivers, driverId, surname), by = "driverId") %>%
  filter(year == 2004 | year == 2019) %>%
  filter(circuitRef == "monza" | circuitRef == "hungaroring" | circuitRef == "suzuka") %>%
  group_by(year, lap, circuitId) %>%
  filter(milliseconds == min(milliseconds)) %>%
  ggplot(aes(x = lap, y = milliseconds, group = year, color = interaction(circuitRef, year))) +
  geom_line(aes(text = sprintf("Giro: %d<br>Tempo: %s<br>Pilota: %s", lap, time, surname))) +
  labs(color = "Circuit")
ggplotly(vs_lap, tooltip = "text")
```

Confrontando l'andamento di tre gare delle stagioni 2004 e 2019 (viene considerato il miglior tempo giro per giro) è interessante rilevare anche altri cambiamenti nelle prestazioni, in particolare nel comportamento degli pneumatici.

Isolando il tracciato di Monza si coglie il primo aspetto interessante: le auto del 2019 sono più performanti nei primissimi giri. Questo è dovuto al fatto che gli pneumatici sono lisci e composti da una mescola di mastice più morbida che accumula il calore necessario per la massima aderenza in tempi minori e garantisce una miglior aderenza sul tracciato. Non appena gli pneumatici delle auto del 2004 arrivano a temperatura ottimale vi è il netto superamento in termini prestazionali

Isolando invece i tracciato di Suzuka e l'Hungaroring si nota come l'alleggerimento dell'auto, dovuto al consumo di carburante, supera, in termini prestazionali, l'usura dello pneumatico. Questo, combinato con l'ottimo carico aerodinamico delle monoposto 2019 ne migliora le prestazioni con il passare dei giri.
Le vetture del 2004, al contrario, bilanciano la perdita di aderenza dovuta all'usura con la perdita di peso, mentre la mancanza di aderenza aerodinamica ne fa peggiorare le performance nel fine gara.
...
Il principale obbiettivo di questa sezione è costruire un indice di pregio dei piloti basato interamente sui dati raccolti.
</div>
...
## Dati aggiuntivi per il sistema di punteggio
E' necessario stabilire alcuni dati da inserire nella tabella dei punteggi che saranno poi utili nel momento in cui dovrà essere stabilito il miglior pilota della F1.
...
+ il numero di piloti della stagione
...
1. Trovare le classifiche dopo l'ultima gara di ogni stagione e isolare il pilota vincitore per ogni anno
```{r champions}
champions <- 
...
Ci sono stati tre campioni della F1 che per vari problemi non hanno partecipato all'ultima gara della stagione, l'autore del dataset ha deciso di non inserire in __races__ un entry per questi piloti. Anche questo problema è stato corretto in precedenza.

## Numero di vittorie
Il primo indice calcolato è stato il numero di vittorie rispetto al numero di gare effettuate $$winsRate = \frac {wins}{starts}$$

Per trovare il numero di gare effettuato è bastato contare in __results__ il numero di apparizioni di ogni id corrispondente ad un pilota campione del mondo:
```{r}
champions <- champions %>%
    left_join(
...
```
$~$
Il numero di vittorie è clacolabile anch'esso a partire dal df __results__:
```{r}
champions <- champions %>%
  left_join(
    left_join(results, select(races, raceId, year), by = "raceId") %>%
...
Un indice interessante per valutare le performance di un pilota è il rapporto $$ability = \frac {punti\;pilota}{punti\;squadra} $$ che fornisce una misura del grado di bravura del pilota rispetto al compagno di squadra (importante è sottolineare che i piloti utilizzano due vetture identiche e varia solo l'assetto in funzione del circuito).
$~$
Per calcolare tale indice vengono inizialmente aggiunte delle informazioni al df __champions__:
```{r}
champions <- champions %>%
...
```
$~$
Dal __point_scoring_system__ e dal df precedente è evidente come nei primi 8 anni della F1 non si sia disputato il campionato costruttori. Il campionato costruttori si svolge parallelamente a quello piloti e consta nel cumulo dei punti di TUTTI i piloti che corrono per una scuderia.
Dato che l'indice introdotto in precedenza necessita del numero di punti nel campionato costruttori viene fatta manualmente la proiezione dei punti che i piloti avrebbero portato al proprio costruttore.
$~$
Inizialmente è cruciale sapere quali sono i costruttori per cui i campioni del mondo gareggiavano:
```{r}
...
```
$~$
Di tutte le gare individuate vengono filtrate quelle in cui i piloti sono arrivati almento quinti (come si può notare dalla tabella __point_scoring_system__, tra il 1950 e il 1957 solo i piloti arrivati almeno quinti ottenenvano dei punti) e la variabile _position_ viene convertita in modo da essere equivalente al nome delle colonne in point_scoring_system:
```{r}
fill_the_gap <- fill_the_gap %>%
  filter(position <= 5) %>%
...
```
$~$
Sapendo ora la posizione dei piloti è necessario associare i punti guadagnati:
```{r}
fill_the_gap <- fill_the_gap %>%
...
```
$~$
Avendo ora i punti che ogni pilota della scuderia ha accumulato, è sufficiente raggruppare in base all'anno e fare la somma dei punti di ogni pilota di un costruttore per ottenere i punti totali:
```{r message=FALSE}
fill_the_gap <- fill_the_gap %>%
  group_by(year) %>%
...
```
## Punti cumulati
Un'indice di efficienza di un pilota è sicuramente il rapporto tra i punti cumulati in carriera e il numero di punti che il pilota ha avuto a disposizione: $$perfectionRate = \frac {punti\;carriera}{punti\;disponibili}$$.
...
```
$~$
Combinando le informazioni contenute in __champions__, __results__ e __point_scoring_system__ si possono ottenere i punti cumulati che un pilota avrebbe potuto guadagnare durante la carriera:
```{r}
champions <- champions %>%
  left_join(
    left_join(results, select(races, raceId, year), by = "raceId") %>%
...
```
$~$
Lavorando ora su __champions__ e __results__ è possibile ricavare i punti che un pilota ha ottenuto in carriera:
```{r}
champions <- champions %>%
  left_join(
    left_join(results, select(races, raceId, year), by = "raceId") %>%
...
## La classifica dei campioni
E' stato deciso si costruire calcolare il punteggio dei piloti facendo la media ponderata degli indici calcolati:
$$score = championship \times 0.3 + dominion \times 0.25 + perfectionRate \times 0.15 + winsRate \times 0.2 + ability \times 0.1 $$
```{r}
best <- champions %>%
  mutate(rank = (championship * 0.3 + dominion * 0.25 + perfectionRate * 0.15 + winsRate * 0.2 + ability * 0.1)) %>%
...
```
Dato che la classifica è applicata a tutti i campioni del mondo, stagione per stagione, è possibile avere un pilota con score differenti a seconda della stagione di riferimento. Massimizzando gli score dei piloti è possibile ottenere una classifica ridotta e più efficace:
```{r, out.height = "100%", out.width = "100%"}
best %>%
  group_by(driver) %>%
...
best %>%
  group_by(driver) %>%
  filter(rank == max(rank)) %>%
  ggplot(aes(x = reorder(driver, -rank), y = rank)) +
  geom_col() +
  theme_ipsum() +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("")
...
Come pronosticabile le prime due posizioni sono occupate da Schumacher e da Hamilton. In questa classifica Hamilton ha vinto anche la stagione corrente in base alle gare fino ad ora disputate, se dovesse avere delle prestazioni eccezionali potrebbe passare in testa alla classifica.
Una prossima versione del ranking potrebbe contenere il numero di Pole position dei piloti, ma per una mancanza online dei dati molto vecchi non è semplice risalire stagione per stagione a tale informazione.
