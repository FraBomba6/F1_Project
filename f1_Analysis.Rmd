...
```
Un analisi non contestualizzata del grafico precedente potrebbe far pensare che le auto della stagione 2004 siano più veloci delle auto del 2019 sul giro secco. In realtà i tempi sopra riportati sono influenzati da un fattore esterno alle pure performance delle auto, in particolare i tempi riportati nel grafico sono influenzati dalle performance pneumatici.
Nel 2004 si utilizzavano ancora pneumatici con battistrada intagliato e una mescola nettamente più dura (meno aderenza ma meno usura) rispetto alle auto del 2019, questo garantiva un usura inferiore durante la gara che risultava nei giri finali in una buona aderenza; questa, combinata con il peso ridotto dell'auto, per la presenza nel serbatoio di una quantità esigua di carburante, risultava in performance incredibili nei giri finali della __gara__. Le auto degli ultimi 5 anni, al contrario, sviluppando una maggior potenza motore, necessitano di un'aderenza di altissimo grado il che si converte nell'utilizzo di gomme con mescole morbide e battistrada liscio. Tuttavia questo porta l'usura degli pneumatici ad essere molto alta e costringe i piloti ad attuare tecniche di riduzione dell'usura che si traducono in tempi sul giro maggiori delle reali capacità di pneumatici e di motore.

## Tempi di qualifica
Mentre in gara si attuano delle strategie di conservazione di pneumatici e motore, in qualifica si cerca di massimizzare le prestazioni sui singoli giri. Questo permette di avere un riscontro più veritiero di quelle che sono le prestazioni delle monoposto
```{r , out.height = "100%", out.width = "100%"}
suzuka_q <- qualifying %>%
  left_join(select(races, raceId, circuitId, year), by = "raceId") %>%
  left_join(select(circuits, circuitId, circuitRef), by = "circuitId") %>%
...
suzuka_q$milliseconds = toSeconds(suzuka_q$time)
...
suzuka_q %>%
  ...
  theme(axis.text.x = element_text(angle = 90))
```
Come pare evidente dalla tabella precedente e dal relativo grafico, i tempi in qualifica sono nettamente migliori nel 2019 rispetto al 2004.

E' interessante osservare come tra il 2004 e il 2014 le prestazioni delle auto abbiano avuto una diminuzione (non si considerano il 2008 e il 2017 perché qualifiche svolte su circuito bagnato). Fino al 2004 lo sviluppo tecnico delle auto non ha avuto limiti ma solo regolamentazioni varie. Alcuni incidenti nei primi anni 2000 e le impressionanti prestazioni delle vetture del 2004 hanno spinto la FIA ad effettuare drastici cambiamenti nei regolamenti tecnici per aumentare la sicurezza dei piloti alla guida delle vetture. Una sorta di stabilità del regolamento tecnico raggiunta nel 2014 ha permesso ai costruttori di mantenere standard molto alti di sicurezza e allo stesso tempo di migliorare gli aspetti prestazionali delle vetture
